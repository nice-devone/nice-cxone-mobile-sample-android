 ...
 *
 *
 * Method `registerDefault` has been added to avoid hard errors during deserialization.
...
class RuntimeTypeAdapterFactory<T> private constructor(
    ...
    /**
     * Registers `value` to be default when no other deserializer can handle this type.
     */
    fun registerDefault(value: T): RuntimeTypeAdapterFactory<T> {
        defaultValue = value
        return this
    }
    ...
    override fun <R> create(gson: Gson, type: TypeToken<R>?): TypeAdapter<R>? {
        ...
        return object : TypeAdapter<R>() {
            ...
            override fun write(out: JsonWriter, value: R) {
                ...
                val delegate// registration requires that subtype extends T
                        = subtypeToDelegate[srcType] as TypeAdapter<R>? ?: throw JsonParseException("cannot serialize " + srcType.name
